...
def assert_reasoning_sequence(
    reasoning_steps: List[BaseReasoningStep],
    expected_sequence: List[Type[BaseReasoningStep]]
) -> None:
...
def assert_tool_called(
    ...
    tool_calls = [
        ...
        if isinstance(step, ActionReasoningStep) and step.action == tool_name
    ]
    ...
    if expected_input is not None:
        ...
        assert actual_input == expected_input, (
            f"Tool input mismatch. Expected: {expected_input}, Got: {actual_input}"
        )
