"""Tests for LangGraph SimpleReActAgent using TrajectoryMockLLMLangChain."""
...
async def test__single_tool_call__returns_response_with_one_source() -> None:
    ...
    assert "Weather in Tokyo" in result["response"] or "sunny" in result["response"]
    assert len(result["reasoning"]) == 2  # Two reasoning steps
...
async def test__tool_not_found__adds_error_to_sources() -> None:
    ...
    assert len(result["sources"]) >= 0  # May have error message as source
...
async def test__multiple_tools_available__selects_correct_tool() -> None:
    ...
    assert "current time" in result["response"].lower() or "time" in result["response"].lower()
