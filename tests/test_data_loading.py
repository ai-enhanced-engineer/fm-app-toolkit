...
def test_gcp_document_repository_with_key(mock_gcs_reader):
    ...
    repo = GCPDocumentRepository(bucket="test-bucket", key="path/to/file.txt")
    documents = repo.load_documents()
...
def test_gcp_document_repository_with_prefix(mock_gcs_reader):
    ...
    repo = GCPDocumentRepository(bucket="test-bucket", prefix="documents/")
    documents = repo.load_documents()
...
def test_gcp_document_repository_with_service_account(mock_gcs_reader):
    ...
    repo = GCPDocumentRepository(bucket="test-bucket", key="file.txt", service_account_key=service_account_key)
    documents = repo.load_documents()
...
def test_gcp_document_repository_handles_load_error(mock_gcs_reader):
    ...
    repo = GCPDocumentRepository(bucket="test-bucket", key="file.txt")
